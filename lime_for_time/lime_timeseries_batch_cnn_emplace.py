...
class LimeTimeSeriesExplainer(object):
    ...
    def explain_instance(self,
       ...
       """
        ...
        permutations, predictions, distances = self.__data_labels_distances(
            timeseries=timeseries_instance, other_instances=other_instance, classifier_fn=classifier_fn,
            num_samples=num_samples, num_slices=num_slices, replacement_method=replacement_method)
    ...
    def __data_labels_distances(cls,
        ...
        values_per_slice = len(timeseries[0]) // num_slices
        deact_per_slice = np.random.randint(1, num_slices + 1, num_samples)
        perturbation_matrix = np.ones((num_samples, num_slices))
        features_range = range(num_slices)
        original_data = []  # timeseries.copy()]

        if len(timeseries) < len(other_instances):
            ratio = len(other_instances) // len(timeseries)
            samples_for_ratio = ratio * len(timeseries)
            other_instances = other_instances[:samples_for_ratio:ratio]
        else:  # len(timeseries) >= len(other_instances)
            ratio = len(timeseries) // len(other_instances)
            samples_for_ratio = ratio * len(other_instances)
            timeseries = timeseries[:samples_for_ratio:ratio]

        for i, num_inactive in enumerate(deact_per_slice, start=0):
            ...
            if i == 0:  # unperturbed has to be in first example
            ...
            for idx in inactive_idxs:
                ...
                end_idx = min(end_idx, len(timeseries[0]))

                if replacement_method == 'emplace':
                    tmp_series[:, start_idx:end_idx, 0] = other_instances[:, start_idx:end_idx, 0]
                    ...
                    tmp_series[:, start_idx:end_idx] = np.random.uniform(
                        tmp_series[:, start_idx:end_idx].min(),
                        tmp_series[:, start_idx:end_idx].max(),
                        size=tmp_series[:, start_idx:end_idx].shape)
                elif replacement_method == 'total_noise':
                    # use random noise as inactive
                    tmp_series[:, start_idx:end_idx] = np.random.uniform(
                        tmp_series.min(),
                        tmp_series.max(),
                    ...
                    # use random noise as inactive
                    tmp_series[:, start_idx:end_idx, 0] = np.min(tmp_series[:, start_idx:end_idx, 0])
        ...
        for sample in original_data:
            predictions_all.append(classifier_fn(sample))

        predictions = np.mean(np.array(predictions_all), axis=1)
